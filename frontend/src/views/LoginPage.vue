<template>
  <main class="page-wrap">
    <section class="max-w-md mx-auto ui-card p-6 sm:p-7 fade-up">
      <div class="mb-6">
        <p class="badge mb-3">{{ t('authPages.welcomeBack') }}</p>
        <h1 class="page-title !text-3xl mb-1">{{ t('authPages.signIn') }}</h1>
        <p class="page-subtitle">{{ t('authPages.signInSubtitle') }}</p>
      </div>
      ...
      <form class="space-y-4" @submit.prevent="handleLogin">
        ...
        <div>
          ...
          <div class="password-wrap">
            <input v-model="form.password" :type="showPassword ? 'text' : 'password'" required minlength="8" class="field field--password" />
            <button
              type="button"
        ...
        <button type="submit" :disabled="authStore.loading" class="btn-primary w-full">
          {{ authStore.loading ? t('authPages.signingIn') : t('authPages.signInAction') }}
        </button>
      </form>

      <div class="my-5 text-center text-sm text-[color:var(--ink-500)]">{{ t('authPages.or') }}</div>
      ...
      <p class="text-sm mt-6 text-center text-[color:var(--ink-600)]">
        ...
        <RouterLink to="/register" class="font-semibold underline">{{ t('authPages.createOne') }}</RouterLink>
      </p>
    </section>
  </main>
</template>
...
async function handleGoogleCredential(response) {
  if (!response?.credential) {
    googleError.value = t('authPages.googleNoCredential')
    return
  }
  try {
...
async function renderGoogleButton() {
  ...
  if (!clientId) {
    googleError.value = t('authPages.googleClientMissing')
    return false
  }
  ...
  try {
    ...
    googleError.value = t('authPages.googleScriptFailed')
    return false
  }
